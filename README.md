...
```
analysis/
  boundary_detector.py        Main pipeline (Ridge → Otsu → morphology → contour)
  boundary_comparison.py      Multi-method boundary comparison
  multi_index_boundary.py     Multi-vegetation-index variants
  gmm_boundary.py             GMM (k=2) baseline on NDVI
  threshold_comparison.py     Thresholding ablation
  boundary_with_reference.py  Overlay detected boundary vs reference shapefile
  paper_boundary_figure.py    Paper Fig. 2 (Landsat / K-means / GMM / Ours)
  paper_single_figure.py      Paper Fig. 3 (boundary vs phytogeographic line)
  compute_metrics.py          Paper Table 1 (R^2, correlation, smoothness gain)
  feature_extractors/
    vegetation_indices.py     NDVI / SAVI / EVI / NDWI / BSI / NDBI
downloaders/
  google_embedding.py         AlphaEarth Satellite Embedding V1 (annual)
  landsat_savi.py             Landsat 8/9 Collection 2 L2 with NDVI/SAVI
  kml_region_downloader.py    Download by KML polygon (Algeria, custom AOIs)
```
